...
def formdf(psth_proj,linregCols,condstrlist,binedge,bsref='off'):
    ...
    for tt in range(psth_proj.shape[1]): # loop through timepoints
        psth_proj_df_ttall = pd.DataFrame()
        for cc in range(psth_proj.shape[2]): # loop through conditions                
        ...
        if bsref=='on':
            ...
            ### to the respLabel axis
            psth_proj_df_tthit = psth_proj_df_ttall[psth_proj_df_ttall['resp']=='hit'][['respLabel']].mean().values
            psth_proj_df_ttmiss = psth_proj_df_ttall[psth_proj_df_ttall['resp']=='miss'][['respLabel']].mean().values
            if psth_proj_df_tthit>=psth_proj_df_ttmiss:
                psth_proj_df_ttall['respLabel_bscrct'] = psth_proj_df_ttall['respLabel'].values-psth_proj_df_ttmiss
            else:
                psth_proj_df_ttall['respLabel_bscrct'] = psth_proj_df_ttmiss-psth_proj_df_ttall['respLabel'].values
            try :
                coeff_temp,pval_temp,evalparam = glmfit(psth_proj_df_ttall,['respLabel'],['resp'],'gaussian') 
            except RuntimeWarning:
                coeff_temp = pd.DataFrame([[np.nan]*2],columns=['coef_resp'])
                pval_temp = pd.DataFrame([[np.nan]*2],columns=['pval_resp'])
            psth_proj_df_ttall['resp-slope'] = coeff_temp['coef_resp'].values[0]  
            psth_proj_df_ttall['resp-slope-Pval'] = pval_temp['pval_resp'].values[0]            
...
bootstrapTims =100#100#  
resolutionred =5# 5 # reduce temporal resolotion of the decoding by this scale, original temporal resolution:0.0

# 3 task related variables
psthGroupCols = ['trialMod','snr-shift','respLabel']
filterdict = {'trialMod':['a','av'],'respLabel':['hit','miss'],'AVoffset':[90,120]}
linregCols = ['snr-shift','trialMod','respLabel']
...
if __name__ == '__main__':
    for alignKeys in ['_align2DT']: # '_align2coo' '_align2DT'
        for STRFstr in ['notsig','sig',]:#['all','notsig','sig']:
            for spkLabel in ['all']: #['all','RS/FS','positive','triphasic']:   
                #for clsNo in [0,1]: #      
                    namstrallSU = alignKeys+'_Hit+Miss=A+AV_ttestfilteredUnits_ProjonModXsnrXChoice_STRF'+ STRFstr+'_spkwavShape-'+spkLabel.replace('/','or')+'_nonoverlaptimwin50msbinRawPSTH'+'_AVoffset90+120'+'_sample30noreplacement_lessWu-sess'
                
                    psth_proj_df_BSall = pd.DataFrame()
                    for bs in range(bootstrapTims):
                        for Monkey,Date in MonkeyDate_all.items():   
                            # different snr filter conditions for each monkey
                            if Monkey=='Elay':
                               filterdict['snr'] = [-15,-10]
                            if Monkey=='Wu':
                               filterdict['snr'] = [-5,0]   
                            # if want to filter out specific spike waveform              
                            ...
                            # filter neurons not drifted and significantly modified by any task related variables, auditory/visual/snr/avmod
                            ...
                            # df_avMod_all_sig = df_avMod_all_sig_unique.copy()
                            ...
                            for Date_temp in Date: 
                                ...
                                ######## modify line80-81 in reshapePSTH when switch input files
                                ...
                                binedge_all = np.arange(-1,1,bin) #align2coo or align2DT psth window is [-1,1]
                                frateNO = np.arange(min(range(len(np.arange(-1,1,bin))), key=lambda i: abs(np.arange(-1,1,bin)[i] - timwinStart[0])) ,
                                                    min(range(len(np.arange(-1,1,bin))), key=lambda i: abs(np.arange(-1,1,bin)[i] - timwinStart[1]))+1,1)     
                    ...
                    # ################### plot projection on the respLabel axis difference between hit and miss condition averaged across SNR and mod              
                    psth_proj_df_BSall_mean = psth_proj_df_BSall.groupby(by=['Monkey','time','resp','bs','shuffled'])[['respLabel','resp-slope']].mean().reset_index()
                    psth_proj_resp_hit = psth_proj_df_BSall_mean[psth_proj_df_BSall_mean['resp']=='hit'].sort_values(by=['Monkey','time','bs','shuffled'])
                    psth_proj_resp_miss = psth_proj_df_BSall_mean[psth_proj_df_BSall_mean['resp']=='miss'].sort_values(by=['Monkey','time','bs','shuffled'])
                    psth_proj_respdiff = psth_proj_resp_hit[['Monkey','time','bs','shuffled']].copy()
                    psth_proj_respdiff['respDiff'] = np.abs(psth_proj_resp_hit['respLabel'].values-psth_proj_resp_miss['respLabel'].values)
                    ...
                    sns.lineplot(psth_proj_respdiff[psth_proj_respdiff['shuffled']=='true label'],x='time',y='respDiff',hue='Monkey',hue_order=['Elay','Wu'],palette=['magenta','royalblue'],ax=axess,estimator='mean', errorbar=('ci', 95))               
                    sns.lineplot(psth_proj_respdiff[psth_proj_respdiff['shuffled']=='shuffled label'],x='time',y='respDiff',hue='Monkey',hue_order=['Elay','Wu'],palette=['plum','lightsteelblue'],ax=axess,estimator='mean', errorbar=('ci', 95))               
                    ...
                    axess.set_xlabel('Time (s; relative to decision time onset)',**font_properties)
                    # axess.set_xlabel('Time (s; relative to joystick onset)',**font_properties)
                    axess.set_ylabel('Hit/Miss separation',**font_properties)
                    axess.tick_params(axis='both', which='major', labelsize=fontsizeNo-4) 
                    axess.set_ylim([0,8])
                    # axess.set_xlim([-0.75,0])
                    fig.tight_layout()
                    fig.savefig(figSavePath+'Popprjcton_RespAxis_Hit-Missdistance_'+namstrallSU+'_2monk_2swin.'+figformat)
                    plt.close(fig)             
